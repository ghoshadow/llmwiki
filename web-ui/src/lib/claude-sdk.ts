...
export function createClaudeSessionConfig(
  ...
  const prompts: Record<typeof task, string> = {
    ingest: [
      ...
      "3. For each topic, use create_page to create a wiki page with proper frontmatter",
      ...
      "6. Update the index if appropriate categories are provided",
    ...
    query: [
      ...
      "Follow these steps:",
      "1. Use search_pages to find relevant content",
      "2. Use read_page to get full details of the most relevant pages",
      "3. If needed, use get_backlinks to find related pages",
    ...
    lint: [
      ...
      "Follow these steps:",
      "1. Run lint_contradictions to get a comprehensive lint report",
      "2. Run find_broken_links to find any broken wikilinks",
      "3. Run find_orphans to find orphan pages",
      "4. Check that index entries match actual pages",
      "5. Compile a complete report of all findings",
...
export function getWikiToolDefinitions() {
  return {
    ...
    get_index: {
      description: "Get the full knowledge index",
      parameters: { type: "object", properties: {} },
    },
    get_log: {
      description: "Get operation log entries",
      parameters: {
        type: "object",
    ...
    get_backlinks: {
      description: "Find pages linking to a given page",
      parameters: {
        type: "object",
...
export function buildIngestSystemPrompt(): string {
  return [
    ...
    "3. For each topic, use create_page with:",
...
export function buildQuerySystemPrompt(): string {
  return [
    ...
    "2. Use read_page on the most relevant results to get full content",
    "3. If needed, use get_backlinks to discover related pages",
...
export function buildLintPrompt(_request: LintRequest = {}): string {
  return [
    "Run a comprehensive quality check on the wiki.",
    "Check for: broken links, orphan pages, missing frontmatter, invalid tags,",
...
export function buildLintSystemPrompt(): string {
  return [
    ...
    "",
    "Workflow:",
    "1. Run lint_contradictions for a comprehensive check",
    "2. Run find_broken_links to identify broken wikilinks",
    "3. Run find_orphans to find pages with no incoming links",
    "4. Use get_index and list_pages to cross-reference the index with actual pages",
    "5. Compile all findings into a clear report grouped by severity",
